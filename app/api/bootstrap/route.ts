...
export async function GET(request: Request) {
  try {
    ...
    const grouped = new Map<string, { id: string; type: string; title: string; body: string; happenedAt: string; author: string; photos: { id: string; label: string; status: "uploaded" | "ready" | "error" }[]; updatedAt: string }>();
    for (const row of rows) {
      ...
      if (row.photo) current.photos.push({ id: row.photo.id, label: row.photo.altText || `Foto dari cerita tanggal ${row.entry.happenedAt}`, status: row.photo.status as "uploaded" | "ready" | "error" });
