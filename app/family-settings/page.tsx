...
export default function FamilySettingsPage() {
  ...
  const { familyName, child, ownerEmail, partnerEmail, partnerStatus, membershipRole, updateFamilyName, signOut, leaveFamily, startExport, exportStatus } = useDemo();
  ...
  return <PageFrame className="settings-page"><div className="settings-header"><div className="settings-header__intro"><BackLink href="/timeline" /><div className="settings-header__title"><span className="eyebrow">Pengaturan</span><h1>{familyName || "Jurnal keluarga"}</h1></div></div><span className="private-pill"><LockKey size={14} weight="bold" /> Jurnal privat</span></div><section className="settings-section"><span className="settings-section__title">Profil keluarga</span><div className="settings-card"><form className="form-stack" onSubmit={saveFamilyName}><TextField label="Nama keluarga" id="familyName" value={draftFamilyName} onChange={setDraftFamilyName} placeholder="Contoh: Keluarga Prakoso" maxLength={160} /><Button type="submit" loading={savingFamilyName}>Simpan nama keluarga</Button></form></div></section><section className="settings-section"><span className="settings-section__title">Anggota keluarga</span><div className="settings-card"><div className="family-card__top"><Avatar name={familyName || "Keluarga"} size="lg" /><div className="family-card__copy"><strong>{familyName || "Jurnal keluarga"}</strong><span>Jurnal bersama untuk momen kecil</span></div></div><div className="member-list"><div className="member-row"><div className="member-row__identity"><Avatar name="Baba" size="sm" /><div><strong>Baba</strong><span>{ownerEmail || "Email pemilik belum tersedia"}</span></div></div><span className="member-status">Aktif</span></div><div className="member-row"><div className="member-row__identity"><Avatar name="Bubu" size="sm" /><div><strong>Bubu</strong><span>{partnerEmail || "Belum diundang"}</span></div></div><span className={`member-status ${partnerStatus === "pending" ? "member-status--pending" : ""}`}>{partnerStatusLabel}</span></div></div><button className="setting-action" onClick={() => router.push("/invite-partner")}><span><UserPlus size={17} /> Undang pasangan</span><ArrowRight size={17} /></button></div></section><section className="settings-section"><span className="settings-section__title">Media</span><div className="settings-card storage-card"><div className="storage-card__top"><div><strong>{storage.used}</strong><span> dari {storage.quota}</span></div><span>{storage.percent}% terpakai</span></div><div className="progress-track" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={storage.percent} aria-label={`${storage.used} dari ${storage.quota} terpakai`}><span style={{ width: `${storage.percent}%` }} /></div><div className="storage-card__footer"><span>Foto disimpan privat di perangkat kamu</span><Button variant="secondary" onClick={exportPhotos}>{exportStatus === "queued" ? "Menyiapkan..." : exportStatus === "ready" ? "Export siap" : <><DownloadSimple size={16} /> Export foto</>}</Button></div></div></section><section className="settings-section"><span className="settings-section__title">Akun</span><div className="settings-links"><a className="settings-link" href="#privacy"><LockKey size={18} /> Kebijakan privasi <ArrowRight size={16} /></a><button className="settings-link settings-link--danger" onClick={handleLeaveFamily}><SignOut size={18} /> Keluar dari jurnal keluarga <ArrowRight size={16} /></button><button className="settings-link" onClick={handleSignOut}><SignOut size={18} /> Keluar dari akun <ArrowRight size={16} /></button></div></section>{toast && <Toast><Check size={16} weight="bold" />{toast}</Toast>}</PageFrame>;
}
