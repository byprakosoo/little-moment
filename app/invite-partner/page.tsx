...
export default function InvitePartnerPage() {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); if (!email.includes("@")) { setError("Masukkan email pasangan yang valid."); return; } setError(""); setLoading(true); try { await invitePartner(email.trim()); setSent(true); } catch (inviteError) { setError(inviteError instanceof Error ? inviteError.message : "Undangan belum bisa dikirim. Coba lagi."); } finally { setLoading(false); } };
